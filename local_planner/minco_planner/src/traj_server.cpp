...
void displayTrajPoint(std::vector<Eigen::Vector3d> &path)
{
  pcl::PointCloud<pcl::PointXYZI>::Ptr path_points;
  path_points.reset(new pcl::PointCloud<pcl::PointXYZI>());
  ...
  for (int i = 0; i < int(path.size()); i++)
  {
    ...
    point.y = path[i](1);
    point.z = path[i](2);
    point.intensity = float(i) / float(path.size());

    path_points->points.push_back(point);
  }

  sensor_msgs::PointCloud2 traj_point_msg;
  pcl::toROSMsg(*path_points, traj_point_msg);
  traj_point_msg.header.stamp = ros::Time::now();
  traj_point_msg.header.frame_id = "world";
  traj_point_pub.publish(traj_point_msg);
...
int main(int argc, char **argv) {
  ...
  traj_point_pub = nh.advertise<sensor_msgs::PointCloud2>("/planning/traj_point", 10);
