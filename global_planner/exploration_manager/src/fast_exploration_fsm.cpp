...
void FastExplorationFSM::init(ros::NodeHandle &nh,
  ...
  nh.param("fsm/vaild_num_cube", fp_->vaild_num_cube, 5);
  nh.param("fsm/sample_traj_point", fp_->sample_traj_point, 0.5);
  nh.param("fsm/hybrid_search_radius", fp_->hybrid_search_radius, 15.0);
  nh.param("fsm/unknown_penalty_factor", fp_->unknown_penalty_factor, 2.0);
  ...
  global_path_update_timer_ = nh.createTimer(
      ros::Duration(0.2/*0.2*/), &FastExplorationFSM::globalPathUpdateCallback, this);
...
void FastExplorationFSM::updateTopoAndGlobalPath() {
  ...
  for (auto &cluster : expl_manager_->frontier_manager_ptr_->cluster_list_)
  {
    if (!cluster->is_dormant_ && cluster->is_reachable_)
    {
      cluster_box_max.push_back(cluster->box_max_);
      cluster_box_min.push_back(cluster->box_min_);
    }
  }

  assert(cluster_box_max.size() == cluster_box_min.size());
  ...
  planner_manager_->topo_graph_->updateSkeleton(new_path, cluster_box_max, cluster_box_min, unkownpoints, activepoints, fp_->vaild_num_cube, fp_->sample_traj_point);  // 更新节点
  ...
  int res = expl_manager_->planGlobalPath(odom, vel, unkownpoints, activepoints, fp_->hybrid_search_radius, fp_->unknown_penalty_factor); // 规划全局轨迹
